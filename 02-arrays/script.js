...
const renderCourses = () => {
    ...
    const courseHTML = courses.map(coure => {
        return `
            <li>
                <span>${coure.title} <strong>${coure.price}</strong></span>
                <button onclick="addToCart(${coure.id})" >Додати</button>
            </li>
        `;
    });
...
const renderCart = () => {
    ...
    cartList.innerHTML = cartHTML.length ? cartHTML.join('') 
